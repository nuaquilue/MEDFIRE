...
forest.mgmt <- function(land, harvest, clim, t, out.path){
  ...
  # fix the n of new cells
  # round(dmnd.sawlog*0.1)
  ...
  while(dmnd.sawlog>0){
    ...
    ## Select N ignition points where to start the harvesting interventions
    igni.id <- sample(pextract$cell.id, runif(1,100,1000), replace=F, pextract$p)
    front <- c(front, igni.id)
